...
type FloatingSelectProps = JSX.HTMLAttributes<HTMLButtonElement> & {
  ...
  onChange?: (event: Event & { currentTarget: HTMLElement; target: Element }) => void;
};
...
export function FloatingSelect(props: FloatingSelectProps) {
  ...
  const selectedText = () => {
    const items = childrenAccessor.toArray() as HTMLElement[];
    const selected = items.find((item: HTMLElement) => {
      return item.getAttribute?.("value") === local.value;
    });
  ...
  function handleItemClick(event: MouseEvent) {
    ...
    const menuItem = target.closest("mdui-menu-item");
    ...
    if (menuItem) {
      ...
      if (value !== null && local.onChange) {
        ...
        local.onChange(syntheticEvent as Event & { currentTarget: HTMLElement; target: Element });
      }
      setIsOpen(false);
    }
  }
